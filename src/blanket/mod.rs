use crate::{
    expr::Expr,
    item::{Field, Ident, Order},
};

macro_rules! gen_binops_method {
    ($method:ident) => {
...
pub trait ExprExt<'a>: Sized + Into<Expr<'a>> {
    gen_binops_method!(eq);
    gen_binops_method!(eq as equal);

    gen_binops_method!(ne);
    gen_binops_method!(ne as not_equal);

    gen_binops_method!(gt);
    gen_binops_method!(gt as greater_than);

    gen_binops_method!(ge);
    gen_binops_method!(ge as greater_equal);

    gen_binops_method!(lt);
    gen_binops_method!(lt as less_than);

    gen_binops_method!(le);
    gen_binops_method!(le as less_equal);

    gen_binops_method!(and);
    gen_binops_method!(or);
    gen_binops_method!(like);
    gen_binops_method!(ilike);
